            ...
            if CommaParse is not None:
                for EqualParse in CommaParse.split('='):
                    try:
                        ...
                        if i == 3:
                            self.Angular_Motions[GYRO][YAW] = (self.Gyro[YAW] - (float(EqualParse) * (180 / math.pi)))
                            # if float(EqualParse) > 0.0:
                            #     self.Gyro[YAW] = round((float(EqualParse) * (180 / math.pi)), 5)
                            # elif float(EqualParse) < 0.0:
                            #     self.Gyro[YAW] = round((float(EqualParse) * (180 / math.pi)) + 360.0, 5)
                            #     self.Gyro[YAW] = -self.Gyro[YAW]
                            self.Gyro[YAW] = round((float(EqualParse) * (180 / math.pi)), 5)
                        if i == 5:
                            self.Angular_Motions[GYRO][ROLL] = (self.Gyro[ROLL] - (float(EqualParse) * (180 / math.pi)))
                            self.Gyro[ROLL] = round(float(EqualParse) * (180 / math.pi), 5)
                        i += 1
                    except:
                        pass
                        # print("error reading gyro")
        self.Measures[0][ROLL] = self.Measures[0][ROLL] * .5 + self.Gyro[ROLL] * .5
