...
class MovementCommander:
    ...
    def receiveCommands(self, commandlist):
        ...
        for command in commandlist:
            ...
            print("Wait for supplementary: ", self.WaitForSupplementary)
            print("Is Move Command: ", self.IsMoveCommand)
            print("Yaw offset: ", self.YawOffset)
            print("Sleep 3...")
            time.sleep(3)
            if self.IsMoveCommand:
                print("is move command")
                self.Running = True
                ...
                while self.Running:
                    ...
                    if self.TargetOrPosition == A_GYRO or self.TargetOrPosition == A_POSITION:
                        print("")
                        self.UpdateThrustersPID()
                        self.CheckIfGyroDone()
                        self.Running = self.GyroRunning
                    elif self.TargetOrPosition == A_TARGET:
                        pass  # target finding here
                    elif self.Basic:
